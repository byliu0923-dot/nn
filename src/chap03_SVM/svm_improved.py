...
def plot_decision_boundary(X, y, model, title, filename=None):  # 定义绘制决策边界的函数
    ...
    参数:
        ...
        filename: 保存文件名
    """  # 函数文档字符串，解释参数含义
    h = 0.5  # 设置生成网格的步长，决定背景着色的细腻程度
    
    # 创建网格
    x_min, x_max = X[:, 0].min() - 5, X[:, 0].max() + 5  # 计算 x 轴范围并留出边距
    y_min, y_max = X[:, 1].min() - 5, X[:, 1].max() + 5  # 计算 y 轴范围并留出边距
    xx, yy = np.meshgrid(np.arange(x_min, x_max, h),  # 生成网格坐标矩阵 xx
                         np.arange(y_min, y_max, h))  # 生成网格坐标矩阵 yy
    
    # 获取网格上的预测
    Z = model.predict_proba(np.c_[xx.ravel(), yy.ravel()])  # 将网格展平并合并为特征，预测每个点的决策得分
    Z = Z.reshape(xx.shape)  # 将预测结果还原为网格形状
    
    # 绘制
    fig, ax = plt.subplots(figsize=(10, 8))  # 创建一个 10x8 英寸的画布和子图
    
    # 绘制决策边界等高线
    contourf = ax.contourf(xx, yy, Z, levels=20, cmap=plt.cm.RdBu, alpha=0.6)  # 绘制填充的决策得分等高线，使用蓝红渐变
    ax.contour(xx, yy, Z, levels=[0], linewidths=2, colors='black')  # 绘制得分为 0 的等高线，即分类边界线
    
    # 绘制数据点
    pos = y == 1  # 找出正类样本的掩码
    neg = y != 1  # 找出负类样本的掩码
    ax.scatter(X[pos, 0], X[pos, 1], c='red', marker='o', s=100, label='正类 (1)', edgecolors='k')  # 绘制红色圆点代表正类
    ax.scatter(X[neg, 0], X[neg, 1], c='blue', marker='s', s=100, label='负类 (-1)', edgecolors='k')  # 绘制蓝色方块代表负类
    
    ax.set_xlabel('特征 1 (x1)', fontsize=12)  # 设置 x 轴标签
    ax.set_ylabel('特征 2 (x2)', fontsize=12)  # 设置 y 轴标签
    ax.set_title(title, fontsize=14, fontweight='bold')  # 设置图表标题，加粗显示
    ax.legend(fontsize=11)  # 显示图例，并设置字体大小
    ax.grid(True, alpha=0.3)  # 显示网格线，并设置透明度
    
    plt.colorbar(contourf, ax=ax, label='决策函数值')  # 添加颜色条，说明颜色与得分的对应关系
    plt.tight_layout()  # 自动调整布局，防止元素重叠
    
    if filename:  # 如果指定了文件名
        plt.savefig(filename, dpi=100, bbox_inches='tight')  # 将图表保存为图片，并剪裁边缘多余空白
