...
- 多档位调速原理：通过全局档位变量搭配倍率数组，动态换算实时飞行速度，支持飞行中无感切换
  ...
  ```
## 3. 系统模块设计
本系为了提升代码的可维护性与功能的可扩展性，采用模块化分层设计，构建了 “通信交互层 - 核心控制层 - 功能实现层” 三级架构：以飞行控制模块为核心，整合AirSim仿真通信与键盘交互能力，将各类轨迹与安全功能封装为独立可调用的模块，既实现了控制逻辑的解耦，也为后续功能迭代与问题调试提供了便利。
### 3.1 核心模块概览
